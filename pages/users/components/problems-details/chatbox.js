...
const SenderIcon = styled.span`
  ...
  svg {
    ...
    height: 16px;
    ${(props) =>
      props.type === "user" ? "color: #e53e3e;" : "color: #2c5282;"}
  }
`;

const SenderName = styled.span`
  font-weight: 500;
  ${(props) => (props.type === "user" ? "color: #e53e3e;" : "color: #2c5282;")}
`;
...
const ChatBox = ({ code, problemTitle, problemDescription }) => {
  ...
  const handleCodeSubmit = () => {
    if (code) {
      setMessage(`This is my code:${code}`);
    }
  };

  const handleProblemSubmit = () => {
    if (problemTitle && problemDescription) {
      setMessage(`This is my title:${problemTitle}: ${problemDescription}`);
    }
  };
  ...
  return (
    <>
      ...
      <ChatBoxContainer isVisible={isVisible}>
        ...
        <ButtonContainer>
          <DescriptionButton onClick={handleProblemSubmit}>
            Submit Problem
          </DescriptionButton>
